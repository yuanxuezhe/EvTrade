...
- **目标通过率**：100%（新写的测试必须全过）
- **核心目标**（2026-08-27 基线）：
  - `pytest hq/ server/tests/ tests/` → **116 collected / 116 passed / 0 failed**（数字按 `pytest` 输出如实记录；任何 refactor 不得让 `passed` 数字降低）
  - 旧基线（2026-08-23，71/64/7）已废弃；2026-08-27 曾记录 133/121/12 的历史 fail 已全部清理（P1-1 三刀 + 删 2 个过时测试文件 + cancel fixture 隔离修复）

### 历史失败归属（已清零）

2026-08-27 P1-1 三刀 + cancel fixture 隔离修复后，`test_place_async.py`（6）/ `test_quota_batch.py`（5）/ `test_pos_push_diff.py`（1, 已删）/ `test_v78_skip_rebroadcast.py`（1, 已删）/ `test_orders_cancel.py`（1）全部通过，归因表废弃。

> 若 pytest 再出现 fail，属 regression——先查是否 fixture 期望漂移（broker 字典变更），不是代码 bug 再按 § 八 更新基线并在此登记。
