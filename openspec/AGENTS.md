...
| `2026-08-24-ai-agent-claudedemo` | **🚧 进行中 (squash 合并)** | EvTrade AI 助手架构重做: 后端 spawn `claude -p` 子进程 + 本机 HTTP MCP server (server/ai/), 取代旧 self-built Hermes API server 链路. 删除 server/services/agent/* + server/ws/agent_handler.py + 3 个旧 archive. 前端 AgentPanel UI 不动. claude binary 部署约束. (详见 `archive/2026-08-24-ai-agent-claudedemo/proposal.md` 提交时补) |
