...
> 下表 ✅ 已归档行 = 历史索引（单一事实来源，本文件不重复）。当前无进行中 change。
...
| `2026-08-25-disable-ai-assistant-claude-missing` | **✅ 已归档** | Claude CLI 缺失优雅降级 (2026-08-25): 后端 `GET /api/ai/status` 公开探测 (REQ-AI-007) + WS handler 推 `error`+`agent_complete` 双事件 (REQ-AI-008) + 前端 store 探测 + AgentPanel 浮动按钮灰显 (REQ-FE-539). commit `ed3fd14`+`348be69`+`3a13c03`+`9ff3754`+`372deb9`. (详见 `archive/2026-08-25-disable-ai-assistant-claude-missing/proposal.md`) |
...
| `2026-08-25-chore-p0p1-cleanup` | **✅ 已归档** | 14 项非功能性 P0+P1 清理：pytest testpaths → server/tests+tests + 删 legacy tests/server/ (46 文件) + tests/hq/test_hqserver.py；删 server/requirements.txt + 修 vitest 4.1.9→1.6.0；加 ruff pyproject.toml [tool.ruff] + GitHub Actions ci.yml (backend pytest + frontend build)；补 orders cancel 测试 5 case (happy 48 拒 + 无 order_id + RPC 异常兜底)；删 5 处后端死代码 (place.py format_ts / ai_analysis.py re + threading / quote_snapshots QuoteSnapshot alias / t0/tasks.py user_id_kw + _compute_summary 双签名)；main.py 25 处 startup/shutdown print→log；info/quant 2 处裸 except→queue.Empty；删前端 3 处死代码 (Dashboard STATUS_TYPE / T0Trade formatAmount / format.js formatAmount alias). pytest 81 passed / 7 failed (基线持平). 11 commit: KB sync / 9 code + 1 docs(openspec). (commit `25f838b`+`2ea56d3`+`9ba9e45`+`69e2ca2`+`a3229e4`+`2003fc9`+`d35e7a0`+`8a559fc`+`db60479`+`db25022`; 详见 `archive/2026-08-25-chore-p0p1-cleanup/proposal.md`) |
| `2026-08-25-feat-order-quickqty-fractions` | **✅ 已归档** | OrderForm 委托数量快捷按钮从绝对股数 `[100,500,1000,5000,10000]` 改可用分数 `[1/10,1/5,1/4,1/2,1/1]`. 整手按 `stocksStore.stockTradeUnit(code)` 计算 (买 floor / 卖 ceil, 不超 available). 不足 1 手 → 按钮 disabled. 新增 `availableTradeQty` computed 抽出 `availableText` 单一数据源, 给分数按钮 + applyAvailableToVolume 共用. 12 case 单测覆盖 5 分数 × 2 方向 + ETF unit=1 + 切方向重算. 5 commit: stocks helper / OrderForm 改造 / test / KB+spec merge / 归档. (commit `34e87bf`+`72fda55`+`1fec581`+`5e1ec2e`; 详见 `archive/2026-08-25-feat-order-quickqty-fractions/proposal.md`) |
| `2026-08-25-cleanup-ai-remove` | **✅ 已归档** | 全量删 AI 助手链路: 前端 AgentPanel/AiAnalysis/store/api/router/Sidebar + server/ai/* + server/mcp/* + ai_analysis API + WS agent_channel + HERMES_AGENT_TOKEN 常量 + /api/auth/grant + WS hermesagent 鉴权捷径 + evctl hermes 服务 + init_strategy_exec_env --grant + 4 个 AI 测试 + 删 REQ-FE-537/539/ARCH-008/evctl-hermes spec + 知识库 6 文件 AI 章节. signal_consumer 鉴权改 EVTRADE_SERVICE_TOKEN 直配 (EVTRADE_ADMIN_TOKEN fallback). 代码层 commit `c5ce258`+`81b920f`+`4b8a6da`+`96bd56a`+`ebba4fb`; 收尾 KB 同步 `0edc902` + init 去 grant `ffd9d23` + AGENTS `36a775b` + 归档. (详见 `archive/2026-08-25-cleanup-ai-remove/proposal.md`) |
| `2026-08-30-his-hq-chunked-fetch` | **✅ 已归档** | strategy_exec 历史行情长区间 (≥30 天) broker 单次 fetch 30s 超时 → 拆 10 天/批串行 (`his_hq_chunk_days=10` / `his_hq_chunk_enabled=True`). `_iter_chunks` 纯函数 + `fetch_bars` chunked 调度 (拼凑 sort by stime + 任一段失败整体 raise 原子性). 外部签名不变, 1d 聚合走 chunks 拼完后 aggregator. 10 case 单测. E2E 实测 30 天 3 段 → 4320 根 1m → 18 根 1d (65.9s, broker 无超时). (commit `d2d5d49` 实现 + `5ee08ef` 骨架/单测 + `e12c3f5` 修 aggregator 4 段 case; 详见 `archive/2026-08-30-his-hq-chunked-fetch/proposal.md`) |
| `2026-08-30-his-quote-backfill` | **✅ 已归档** | 数据补全区 → 历史行情补全页: `quote_sync_config` 任务表 (证券/时间区间/已加载游标/自动同步) + `minute_bars` 按日 upsert. 后端自包含 HisHqClient (VWAP 元/股, 假日=成功空, 周末本地跳过+broker `#END_OF_HIS_HQ#` 结束标记秒回) + 配置 CRUD + 按日 `/sync` + on_startup 后台增量补平 (per-stock 锁). 前端 HistoryQuoteCompletion 按日循环 (进行中转圈/失败原因/昨天封顶/串行队列). avg_price 17.4w 行 元/手→元/股 修正. 21 单测. 实施 `4d097a3`..`495aec0`. (详见 `archive/2026-08-30-his-quote-backfill/proposal.md`) |
| `2026-08-30-backtest-exec-log-signal-bars` | **✅ 已归档** | 回测执行日志 `backtest_result.execution_log` 的 `phase="bar"` 段从「逐 bar 全量 (超 2000 采样)」改为**仅触发 buy/sell_signal 的 K 线** — 抽纯函数 `_build_signal_bar_entries(signals, progress_log)` 按 stime 查回 bar_idx/close/position/equity. 6 单测 + E2E. (后续被 `2026-08-30-drop-fullbar-progress` 扩展: best 不再落全量 progress_log/equity_curve). (commit `7238e9a`+`e352dec`+`b58d90e`+`b90c16b`; 详见 `archive/2026-08-30-backtest-exec-log-signal-bars/proposal.md`) |
| `2026-08-30-drop-fullbar-progress` | **✅ 已归档** | 回测详情去掉全量 bar (接上条): `best` 不再落 `progress_log`/`equity_curve` (全量逐 bar 不持久化, progress_log 降级 run 内内存缓冲仅供信号 bar 快照); 前端删「进度」Tab + 权益曲线改用 execution_log 信号 bar 的 {stime, equity} (起点 initial_cash, 去收盘价 series); server /signals progress 新任务自动空. E2E 50 根 2 信号 → best 无全量 bar / 曲线可画. (commit `e0a5cb3`+`87d8211`+`523d78a`+`cdf0ca5`; 详见 `archive/2026-08-30-drop-fullbar-progress/proposal.md`) |
| `2026-08-30-sweep-worker-queue` | **✅ 已归档** | 参数扫描回测**超时**修复 — 两阶段: (1) 提交性能: sweep 建 N 行改批量 INSERT (executemany 单往返, Strategy 查1次) + 端点 DB 写 to_thread 包装 (不阻塞 event loop, 修大扫描吃满前端 15s 超时); (2) 执行队列: 新增 worker.py 回测任务队列 — 每次触发拉 N worker FIFO 有界并发, claim 原子领取 (SKIP LOCKED + 代际+1), per-task wait_for 超时自愈 (复位重跑/超限failed), 代际隔离 (run_generation 列, 孤儿线程写 no-op), finalize 按 backtest_metric_value 取 finished top1 回写 best_params; single+sweep 统一走 /internal/run-batch (run_sweep_batch gather 弃用); strategy_task 加 run_generation 列 (migration 幂等); 18 单测. 已知: worker 池每次触发临时起(内存态), 进程崩溃不自动续跑历史批次 (需重测/重新提交). (commit `ee2c7bb`+`b2c2b91`+`e00709d`+`dc6410e`+`f16c4dc`+`9c5d9d1`+`2208b59`+`83599eb`+`bbede24`; 详见 `archive/2026-08-30-sweep-worker-queue/proposal.md`) |
