...
- [ ] 写 `server/migrations/2026-07-12-add-short-name-to-stocks.py`
  - INFORMATION_SCHEMA 探测 stocks.short_name 是否存在
  - ADD COLUMN short_name VARCHAR(16) NULL
  - 幂等
- [ ] `pip install pypinyin` 写入 server/requirements.txt
- [ ] 跑 migration 验证: `DESC stocks` 有 short_name 列

## 3. ORM + Repo 同步（Commit 3）
- [ ] `server/models/orm.py`: Stock 类加 `short_name = Column(String(16), nullable=True)`
- [ ] `server/repo/stocks.py`:
...
- [ ] `server/scripts/backfill_short_name.py`:
  - 读所有 stocks 行
  - 用 pypinyin lazy_pinyin 转首字母
  - UPDATE short_name WHERE stock_code = ?
  - 进度日志 + dry_run 参数
- [ ] 跑灌入: dry_run 一次 → 实跑一次
- [ ] curl 验证: `?page=1&page_size=20` 返回 total=5529

## 5. 前端全量缓存 + autocomplete（Commit 5）
- [ ] `client/src/stores/stocks.js` 重构:
  - cache: ref([]) 全量
  - pageRows: ref([]) 当前页
  - total: ref(0)
...
- [ ] `client/src/api/stocks.js`: list 加 page/page_size/keyword/sector/is_t0_able 参数
- [ ] `client/src/components/StockCodeAutocomplete.vue`: 新组件
  - props: modelValue / placeholder / disabled
  - emit: update:modelValue / select
  - 三路 OR 筛选: code prefix OR name contains OR short_name prefix
  - el-autocomplete 包装
- [ ] `client/src/views/AdminStockConfig.vue`:
...
- [ ] `openspec/specs/data-model/spec.md` §13: Stock 表加 short_name 字段定义
- [ ] `openspec/specs/stocks/spec.md`: REQ-STOCK-006 真分页 / REQ-STOCK-007 前端缓存 / REQ-STOCK-008 autocomplete
- [ ] 归档 change: `mv openspec/changes/2026-07-12-stocks-cache-and-short-name/ openspec/changes/archive/`

## 7. Validation
- [ ] server 启动校验 (python3 -c "import server.main")
- [ ] DB schema 校验: stocks 9 字段（id 隐含）+ short_name
- [ ] curl 验证: GET /api/stocks?page=1&page_size=20 → 200 + total=5529
- [ ] 前端编译校验: cd client && npm run build
- [ ] 等用户拍板: 是否 push 到 remote