...
- [ ] `openspec/specs/frontend/spec.md` 加 REQ-FE-537 全局 AgentPanel 浮动按钮 + 悬浮对话框
- [ ] `openspec/specs/server-architecture/spec.md` 加 REQ-ARCH-008 hermes agent client + WS gateway + confirmation 协议

## A2 MCP server 骨架（待做）

- [ ] `server/mcp/evtrade_mcp_server.py` — FastMCP 入口 + 启动脚本
- [ ] `server/mcp/tools/read_only.py` — list_positions / get_asset / list_orders / list_trades / get_quote / list_strategies（6 个 read-only tool）
- [ ] `server/mcp/tools/write.py` — save_strategy_script（1 个低危 write tool）
- [ ] `server/mcp/tools/trade.py` — place_order / cancel_order（2 个高危 tool，返回 confirmation_required）
- [ ] `server/mcp/tools/admin.py` — delete_strategy_script / set_user_role / init_trading_day（3 个高危 tool）
- [ ] JWT 注入：tool 接收 `jwt_token` 参数 → 服务端校验 → 注入 user_id 到下游 API 调用
- [ ] 单测 `server/mcp/tests/test_*.py`（mock EvTrade REST API）
...
- [ ] `server/services/agent_confirm.py` — `pending_confirmations: dict[run_id, asyncio.Future]` 状态机
- [ ] `server/api/agent.py` — FastAPI WS 端点 `/api/agent/ws`
  - 接 WS → JWT 校验 → 创建/恢复 session → 接收用户消息 → start_run → 推 WS events
  - 拦截高危 tool call → 推 confirmation_required → 等 Future → 调 MCP tool → 继续 run
- [ ] 单测 `server/tests/test_agent_ws.py`（mock hermes + mock MCP）

## A4 前端 AgentPanel（待做）

- [ ] `client/src/api/agent.js` — WS 客户端（指数退避重连）
- [ ] `client/src/stores/agent.js` — Pinia store（messages / isThinking / pendingConfirmation）
- [ ] `client/src/components/agent/AgentPanel.vue` — 右下角浮动按钮 + 悬浮对话框（fixed bottom-right, 480×600）
- [ ] `client/src/components/agent/MessageList.vue` — 用户/AI/tool 卡片渲染
- [ ] `client/src/components/agent/ConfirmModal.vue` — 高危操作二次确认
- [ ] `client/src/components/agent/ThinkingIndicator.vue` — LLM 推理中旋转图标
- [ ] `client/src/App.vue` 或 `client/src/main.js` — 全局挂载 AgentPanel（所有页面可见）
