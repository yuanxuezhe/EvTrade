...
- [ ] **commit 3 — migration run_generation 列**
...
- [ ] **commit 4 — data_access: claim + 代际守卫**
...
- [ ] **commit 5 — worker.py worker 池 + 堵塞自愈**
...
- [ ] **commit 6 — API /internal/run-batch + 转调**
...
- [ ] **commit 9 — docs(openspec) 归档**
  - spec 合并 + mv 到 archive + AGENTS 行
  - 验收：changes/ 只剩 archive

## 验证 (v6 完成自查)

- [ ] `pytest tests/strategy_exec/ server/tests/ -q` 全过（新增 claim/worker/generation 单测全过）
- [ ] 512 组合提交 < 2s 返回，event loop 不阻塞
- [ ] 端到端：sweep 提交 → 202 → worker 池 FIFO 逐个跑 → 前端逐行状态更新 → 点任务看详情
- [ ] 堵塞自愈：模拟卡死 → stale → 回 queued 重跑 → 超限 failed
- [ ] 孤儿线程写 no-op（代际守卫）
- [ ] single 走统一队列行为不变；live 母单不受影响
- [ ] `sync_schema.py diff` 只 ADD run_generation
