# server-architecture delta — v13 后端 5 层模块契约（NEW spec 初始化）

> change `layered-architecture-and-strategy-master` — 新建 spec
>
> 本 spec 是**首次**定义后端分层架构的契约，所有现有 specs（auth / trading / push / rpc-protocol / data-model / strategy / system-init 等）后续变更都应遵守本 spec 的分层依赖规则。
>
> **不依赖** `2026-07-05-strategy_trade` 的 `services/strategy/` 命名（虽然新加了 strategy service）。本 spec 是横向分层（infra/repo/rpc/services/api），strategy_trade 是纵向业务模块。两者正交。
...
- **不包含** strategy 表 CRUD（远程 `2026-07-05-strategy_trade` 已实现 `server/services/strategy/repository.py`；本 change 不动 strategy 模块）
...
- **继承约束**：`RPClient` MUST 继承 `infra.mq.MessageQueueClient`（v13 起）
...
- **CI 检查**：`find server -name "*.py" -not -path "*/__pycache__/*" -exec wc -l {} + | awk '$1 > 250'` 必须 0 输出
- **迁移期豁免**：迁移中的 facade 文件（如 `server/db.py` 转兼容垫片）暂不计入（但目标 ≤ 50 行）
- **远程豁免**：`server/services/strategy/` 子模块（远程 `2026-07-05-strategy_trade` 实现）需在 v13+ 后续 PR 中逐步拆薄到 ≤ 250 行；本 change 不动
...
- **远程豁免**：`server/services/strategy/` 多个子模块（repository / indicators / flags / regime / grid / engine 等）允许 deep import（远程 facade 模式没建 `__init__.py` 全符号 re-export）；本 change 不动远程 strategy 子模块
...
- **THEN** `wc -l` 检查脚本报错：`server/services/push/dispatcher.py: 280 lines (>250)`
...
- **后续工作**：远程 strategy 模块应建 `__init__.py` 全符号 re-export；本 change 不动

## 不在本 spec 范围

- ❌ 跨层重构的**具体实施计划**（见 `changes/2026-07-06-layered-architecture-and-strategy-master/tasks.md`）
