...
- [x] **Commit 2**: `re-exports` cleanup (22 F401, 单一目的)
...
- [x] **Commit 3**: `infra/rpc/repo` cleanup (8 fixes)
...
- [x] **Commit 4**: `tests/strategy` cleanup (8 fixes)
...
- [x] **Commit 5**: `services/strategy/quote_consumer` cleanup (1 fix)
...
- [x] 全量 `ruff check server/` exit 0
- [x] `python3 -c "from server.main import app"` OK
- [x] `pytest server/tests/strategy/` 75/75 PASS（实际可能更少，按文件）
- [x] `/opsx:verify 2026-07-08-lint-cleanup` 调独立 subagent 验收

## Phase 4: Archive

- [x] 5 commits 都 push 到 origin/master
- [x] 调 `/opsx:archive 2026-07-08-lint-cleanup`
- [x] 调 `/opsx:sync` 更新主 spec（如有 REQ-QUAL 新增）