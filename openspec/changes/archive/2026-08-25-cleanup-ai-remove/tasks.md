...
- [x] **commit 1**: `docs(openspec): 建 cleanup-ai-remove change + spec delta`
...
- [x] **commit 2**: `refactor(frontend): 删 AgentPanel + AiAnalysis + 链路 6 处引用`
...
- [x] **commit 3**: `refactor(server-auth): 删 HERMES_AGENT_TOKEN + /grant + WS 直连免鉴权`
...
- [x] **commit 4**: `refactor(server-ai): 删 server/ai/* + ai_analysis API + WS agent_channel handler`
  - `rm -rf server/ai/`
...
- [x] **commit 5**: `refactor(server-mcp): 删 dead code server/mcp/`
  - `rm -rf server/mcp/`

- [x] **commit 6**: `test(server): 删 4 个 AI 测试文件`
...
- [x] **commit 7**: `chore(scripts): evctl 去 hermes + init_strategy_exec_env 去 hermesagent grant`
...
- [x] **commit 8**: `docs(知识库): 同步删除 8 个 KB 文件的 AI 章节`
...
- [x] **commit 9**: `docs(openspec): AGENTS.md 活跃 change 表清理`
