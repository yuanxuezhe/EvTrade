...
>
> **✅ 2026-08-30 全部完成并归档。** commit `7238e9a`(骨架) + `e352dec`(后端) + `b58d90e`(单测) + `b90c16b`(KB+spec+前端 tag)。
> 验收：`_build_signal_bar_entries` 6 case 全过 / strategy_exec 全套 111 / E2E 真实 run_backtest (50 根 K 线 2 信号) → execution_log bar 段=2、equity_curve=50 全量。
...
- [x] **commit 2 — test(strategy-exec) test_signal_bar_entries.py**
...
- [x] **commit 3 — docs(knowledge+frontend)**
...
- [x] `pytest tests/strategy_exec/ -q` 守 111（含新增 6）
- [x] `execution_log` 中 `phase==="bar"` 条数 = 信号数
- [x] `best.equity_curve` 全量（长度 = K 线数）
