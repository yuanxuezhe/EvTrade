...
> - **2026-08-30 sweep-worker-queue**：`strategy_task` 加 `run_generation INT NOT NULL DEFAULT 0`（migration `2026-08-30-add-strategy-task-run-generation.py`，幂等）。回测任务的**代际 + 重跑计数器**，供 worker 队列堵塞自愈的代际隔离：worker 原子领取（claim）时 +1，该 task 的 progress/result/status 写带 `WHERE run_generation=本次代际`（孤儿线程晚到的写 → no-op），`run_generation > backtest_max_retries` → 标 failed 防无限重跑。存量 task = 0 无影响。详见 [`strategy-exec/spec.md`](../strategy-exec/spec.md) REQ-SE-013
