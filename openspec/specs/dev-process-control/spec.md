...
2. **`docs/`** — 静态沉淀：被覆盖前的 spec 演进史（`specs-history/`）、阶段性大型设计与配套实施计划（`designs/`）、单一外部 API 速查（顶层 `*.md`）。
...
- **THEN** 写入 `docs/specs-history/`，文件名带版本后缀（如 `2026-06-22-t0-quick-redesign-v3.md`）；**不写入** `openspec/changes/archive/`——后者追踪"变更元数据"（proposal + tasks + spec deltas），前者承载"完整 spec 草稿"

#### Scenario: 阶段性大型设计的正确位置

- **WHEN** 沉淀一个跨多个 capability 的大型设计 + 实施计划（如 Vue 交易系统从 0 到 1）
- **THEN** 设计 + 计划对写入 `docs/designs/specs/` 与 `docs/designs/plans/`；**不拆为单个 OpenSpec change**——OpenSpec change 服务小步快跑，这种文件颗粒度不同

#### Scenario: 禁止的操作

- **WHEN** 用户/AI 提议"把 `docs/` 合进 `openspec/`"或反之
