...
**Diff 字段集合**：`{last_vol, vol, avl_vol, cost_price}` — 与 `REQ-PUSH-031`（trd_cfm 增量更新作用域）保持一致。

`stock_name` / `synced_at` / `synced_from` **不参与 diff 判断**：新建行仍按现有路径 add_one + 标记 `synced_from='pos_push'` + 刷 `synced_at`；已有行在 diff 通过时直接返回 None（**不刷 synced_at**，避免心跳污染）。

**实现位置**：`server/services/push/pos.py::handle_pos_push` 入口加本地辅助函数 `_fields_unchanged(existing_pos, incoming) -> bool`：
...
**dispatcher 契约（不变）**：`_broadcast_generic` 已处理 `handler_result is None → return`（见 REQ-PUSH-007 v78 修订）。pos_push 走 `_broadcast_generic` 路径，无需 dispatcher 改动。
