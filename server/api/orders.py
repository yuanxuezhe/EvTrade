...
async def cancel_order(order_no: str, trd_date: str = Query(..., description="8 位数字 YYYYMMDD"),
    ...
    if order.status not in ("48", "49"):
        return CancelResponse(
            ...
            error=f"status {order.status} non-cancellable",
        )
    ...
    cancel_row = Order(
        ...
        user_def=f"CANCEL:{order_no}",    # 关联: cancel → orig
    ...
    if ack_code == 0:
        ...
        if cancelled_qty > 0:
            cancel_trade_id = f"CANCEL-{cancel_order_no}-{int(_time.time())}"
            cancel_trade = Trade(
                trd_date=trd_date,
                order_no=cancel_order_no,
