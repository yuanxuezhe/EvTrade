"""
client.py — RPC 客户端 facade 兼容垫片（phase-2 拆分后）

实现已拆分到 4 个单一职责子模块：
- transport.py (~340) — RPClient 传输层（connect / call / listeners / push / 单例 + 工具）
- parsers_common.py (~80) — 通用响应解析工具（_select_rs / _parse_code_msg / _iter_rows / _to_* / _empty）
- parsers_business.py (~120) — 业务特定解析器（_parse_asset / _parse_orders / _parse_trades / _parse_positions / _parse_order_ack）
...
__all__ = [
    ...
    "RPClient", "_PUSH_CHANNEL",
    "_clean_id", "_wire_dump",
    "_run_handle_push", "_resolve_active_trd_date_safe",
