...
覆盖:
  ...
  2. cancel_order values 仅含 order_id, 不含 market / stock_code
  3. cancel_order headers = "order_id" (单 value)
  4. cancel_order 函数签名仅 order_id, 无 market / stock_code
...
async def test_cancel_order_values_only_order_id(fake_rpc_client):
    """REQ-TRADE-033: values 仅 order_id, 不带 market / stock_code
    (柜台 xtquant API 签名是 (acc, order_id) 二参, 无 market / stock_code 字段)
    """
    with patch("server.rpc.handlers.get_rpc_client", AsyncMock(return_value=fake_rpc_client)):
        with pytest.raises(RuntimeError):
            await cancel_order(order_id="ORD123")
        ...
        # 守门: 不能有 stock_code 字段
        assert "stock_code" not in kwargs["values"]


async def test_cancel_order_headers_single_order_id(fake_rpc_client):
    """REQ-TRADE-033: headers = 'order_id' (单 value), 不是 'market|order_id' 或 'order_id|stock_code'"""
    with patch("server.rpc.handlers.get_rpc_client", AsyncMock(return_value=fake_rpc_client)):
        with pytest.raises(RuntimeError):
            await cancel_order(order_id="ORD123")
        _, kwargs = fake_rpc_client.call.call_args
...
def test_cancel_order_signature_only_order_id():
    """REQ-TRADE-033: cancel_order 函数签名仅 order_id, 无 market / stock_code"""
    import inspect
    ...
    assert "stock_code" not in params, (
        f"签名不应有 stock_code, 实际参数: {params}"
    )